...
object Widget {
    ...
    private val LimitedBy =
        Modifier.padding(start = 50.dp, end = ContentPadding.large).widthIn(max = 400.dp)
    val FabRequiredSize = Modifier.requiredSize(112.dp, 56.dp)
    ...
    operator fun invoke(surface: HazeState, modifier: Modifier = Modifier) {
        ...
        val onAction: (Float) -> Unit = { code: Float ->
            scope.launch {
                when (code) {
        ...
        AnimatedContent(
            ...
            content = { id ->
                ...
                CompositionLocalProvider(LocalNavAnimatedVisibilityScope provides this) {
                    when (id) {
                        ID_PLACEHOLDER -> Spacer(FabRequiredSize)
                        ID_MISTY_TUNES -> MistyTunes(state!!, surface, onAction, clickable)
                        ID_FAB_PLAYER -> FabPlayer(state!!, onAction, clickable)
                    }
                }
            }
        )
