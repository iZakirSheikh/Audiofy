...
value class Chronometer private constructor(private val state: MutableLongState) {
    ...
    fun remaining(duration: Long): Long {
        return when {
            ...
            duration == PlaybackController.TIME_UNSET -> Long.MIN_VALUE
 ...
 */
val NowPlaying2.chronometer: Chronometer
    @Composable
    get() {
        ...
        val chronometer =
            remember { Chronometer(if (position == PlaybackController.TIME_UNSET) 0 else position) }
        ...
        LaunchedEffect(this) {
            ...
            chronometer.raw =
                if (position == PlaybackController.TIME_UNSET) 0 else -current
            // Exit early if:
            // 1. The position is unknown,
            // 2. Playback has completed,
            // 3. Playback is paused/stopped.
            if (position == PlaybackController.TIME_UNSET || !playing)
            ...
            while (true) {
                ...
                if (duration != PlaybackController.TIME_UNSET && current >= duration)
                    current -= duration
