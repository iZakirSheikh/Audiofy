...
private const val TAG = "AudioDirectoryViewModel"

abstract class AbstractAudioDirectoryViewModel<T>(
...
// Albums ViewModel.
class AlbumsViewModel(provider: MediaProvider) : AbstractAudioDirectoryViewModel<Album>(provider) {
...
class GenresViewModel(provider: MediaProvider) : AbstractAudioDirectoryViewModel<Genre>(provider) {
...
// Artists
class ArtistsViewModel(provider: MediaProvider) : AbstractAudioDirectoryViewModel<Artist>(provider) {
...
// Folders
class FoldersViewModel(provider: MediaProvider) : AbstractAudioDirectoryViewModel<Folder>(provider) {
    ...
    override suspend fun fetch(filter: Filter, query: String?): Mapped<Folder> {
        val (ascending, order) = filter
        val data = provider.fetchAudioFolders(query, ascending)
        return when (order) {
