package com.prime.media.old.common.util
...
fun Member(from: Audio, playlistId: Long, order: Int) =
    Playlist.Track(
        ...
        title = from.name,
        subtitle = from.artist,
        artwork = from.albumUri.toString()
    )
...
val Playlist.Track.key get() = uri
 ...
 */
inline val Playlist.Track.toMediaItem
    get() = MediaItem(Uri.parse(uri), title, subtitle, MediaItem.DEFAULT_MEDIA_ID, artwork?.let { Uri.parse(it) })
